...
// Seeds categories + products into the Scrollzea database
...
if (!url) {
  console.error("Set TURSO_DATABASE_URL env var (e.g. file:./scrollzea.db)");
  process.exit(1);
}
...
async function seed() {
  console.log("🌱 Seeding Scrollzea database...\n");

  // ── 1. Seed Categories ──
  console.log("📁 Categories:");
  const existingCats = await db.execute("SELECT slug FROM categories");
  const existingCatSlugs = new Set(existingCats.rows.map((r) => r.slug));
  ...
  for (const cat of CATEGORIES) {
    ...
    await db.execute({
      sql: `INSERT INTO categories (name, slug, icon, description, sort_order, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, 'active', ?, ?)`,
      args: [cat.name, cat.slug, cat.icon, cat.description, cat.sortOrder, now, now],
    });
  ...
  const catsResult = await db.execute("SELECT id, slug, name FROM categories WHERE status = 'active'");
  const catMap = {};
  catsResult.rows.forEach((c) => { catMap[c.slug] = c.id; });

  // ── 3. Seed Products ──
  console.log("\n📦 Products:");
  const existingProds = await db.execute("SELECT slug FROM products");
  const existingProdSlugs = new Set(existingProds.rows.map((r) => r.slug));
  ...
  for (const p of PRODUCTS) {
    ...
    // Add payment options for paid products
    if (p.type !== "FREE") {
      const payOpts = [
        { provider: "RAZORPAY", url: "" },
        { provider: "PAYPAL", url: "" },
        { provider: "WHATSAPP", url: "" },
      ];
      for (const po of payOpts) {
        await db.execute({
          sql: "INSERT INTO payment_options (product_id, provider, payment_url, enabled) VALUES (?, ?, ?, 1)",
          args: [prodId, po.provider, po.url],
        });
      }
    }
  ...
  console.log(`\n✨ Done!`);
  console.log(`   Categories: ${catCount} new`);
  console.log(`   Products: ${prodCount} new`);
  console.log(`\n   Total in DB:`);
  const tc = await db.execute("SELECT COUNT(*) as c FROM categories");
  const tp = await db.execute("SELECT COUNT(*) as c FROM products");
  console.log(`   Categories: ${tc.rows[0].c}`);
  console.log(`   Products: ${tp.rows[0].c}`);
